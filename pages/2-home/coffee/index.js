...
$('.searchBar button').click(function(event) {
  ...
  table.slideUp(function() {
    return $('.tables tbody').empty();
  });
  return $.get("/request?crawl=" + input, function(data) {
    var i, index, item, len, results;
    results = [];
    for (index = i = 0, len = data.length; i < len; index = ++i) {
      item = data[index];
      $('.tables tbody:last-child').append("<tr><td>" + item.title + "</td><td>" + item.link + "</td></tr>");
      if (index === 9) {
        results.push(table.slideDown());
      } else {
        results.push(void 0);
      }
    }
